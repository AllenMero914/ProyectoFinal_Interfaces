...
export const Compras = () => {
  ...
  if (user?.rol === 'VENDEDOR') {
    return (
      <main className="main-content compras-restricted">
  ...
  const [ivaParam, setIvaParam] = useState(0);
  ...
  const load = async () => {
    try {
      ...
      const ivaSetting = params.find((param) => param.clave === 'IVA');
      if (ivaSetting) {
  ...
  const cambiarDetalle = (i, field, value) => {
    const copy = [...detalles];
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="label-row">
              Proveedor
              <button type="button" onClick={() => setNuevoProveedorModal(true)} className="link-btn">
                + Nuevo Proveedor
              </button>
            </label>
          ...
          <div className="quick-actions">
            <button type="button" onClick={() => setNuevaCategoriaModal(true)} className="link-btn">
              <i className="fa-solid fa-folder-plus"></i> Crear nueva categoría de productos
            </button>
            <button type="button" onClick={() => setNuevoProductoModal(true)} className="link-btn">
              <i className="fa-solid fa-plus"></i> Nuevo Producto
            </button>
          </div>

          <h4 className="section-subtitle">Detalles de Compra</h4>
          {detalles.map((d, i) => (
          ...
          <button type="button" onClick={agregarDetalle} className="add-detail-btn">
          ...
          <div className="iva-section">
            <label className="iva-label">
              ...
              />
              <span>Aplicar IVA ({ivaParam}%)</span>
            </label>
