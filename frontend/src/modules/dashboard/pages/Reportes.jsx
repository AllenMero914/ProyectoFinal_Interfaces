...
export const Reportes = () => {
  ...
  if (user?.rol === 'VENDEDOR') {
    return (
      <main className="main-content reportes-restricted">
  ...
  const { dataGrafica, topProductos, transaccionesRecientes, totales } = useMemo(() => {
    ...
    const agregarDato = (fechaStr, monto, tipo, esVenta = false) => {
      if (!fechaStr) return;
  ...
  return (
    <main className="main-content reportes-container">
      
      {/* Header controls */}
      <div className="reportes-header">
        <div>
          <h1 className="reportes-title">Dashboard Analytics</h1>
          <p className="reportes-subtitle">Visión general del rendimiento del negocio</p>
        </div>
      ...
      <div className="report-dashboard-grid">
        ...
        <div className="report-card col-span-1 row-span-1">
          ...
          <div className="progress-bars-container">
            {topProductos.length === 0 ? (
              <p className="empty-state">No hay ventas aún.</p>
            ) : (
              topProductos.map((p, idx) => (
                <div key={idx} className="progress-bar-item">
                  <div className="progress-bar-header">
                    <span>{p.name.substring(0,25)}</span>
                    <span className="progress-bar-qty">{p.qty} unds</span>
                  </div>
        ...
        <div className="report-card col-span-1 row-span-1 donut-card">
          ...
          <p className="donut-label">Margen de ganancia estimado</p>
        </div>
        ...
        <div className="report-card recent-news-card row-span-3">
          <h3 className="card-title">Recently News</h3>
